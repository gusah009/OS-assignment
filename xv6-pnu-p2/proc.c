    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(p->state != RUNNABLE) {
        ...
        for(tmp = ptable.proc; tmp < &ptable.proc[NPROC]; tmp++){
          if (tmp->state == RUNNABLE && getnice(p->pid) > getnice(tmp->pid)) break;
        }
        if (tmp != &ptable.proc[NPROC]) continue;
      }
...
void ps(int pid)
{
  ...
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
    ...
    if (p->state >= 0 && p->state < NELEM(states) && states[p->state])
      ...
      state = "???";
    cprintf("%d\t%d\t%d\t%s\t%s", p->pid, p->parent->pid, p->nice, state, p->name);
    cprintf("\n");
  }
}